...
FSpriteRenderer.prototype.render=function(a,b,c){var d=c.getTopLeftPosition(),f=b.getPosition();b.getShapes();var e=b.getAngle(),h=b.getCurrentImage(),b=c.getZoom()/b.getImageScale(),g=1,j=0,k=0,l=1,m=0,n=0;a.save();b!=1&&a.scale(b,b);if(e!=0){var p=g,q=k,r=m,o=Math.sin(e),e=Math.cos(e),g=p*e-j*o,j=p*o+j*e,k=q*e-l*o,l=q*o+l*e,m=r*e-n*o,n=r*o+n*e;a.transform(g,j,k,l,m,n);q=l/(g*l-j*k);e=-j/(g*l-j*k);r=-k/(g*l-j*k);p=g/(g*l-j*k);o=(k*n-l*m)/(g*l-j*k);g=(g*n-j*m)/(g*l-j*k);k=(f.x-d.x)*c.getZoom()/b;
c=(f.y-d.y)*c.getZoom()/b;m=q*k+r*c+o;c=p*c+e*k+g}else m=(f.x-d.x)*c.getZoom()/b,c=(f.y-d.y)*c.getZoom()/b;a.drawImage(h,m-h.width/2,c-h.height/2);a.restore();renderCount++};function FGame(){window.setInterval(this.frameCount.bind(this),1E3);this.fpsGoal=30;this.instep=this.frames=0;this.cameras=[];this.worlds=[];this.fps=0;this.stepInterval=null}FGame.prototype=new FObservable;FGame.prototype.startSimulation=function(){this.stepInterval=window.setInterval(this.step.bind(this),1E3/this.fpsGoal)};
